...
def test_search_respects_acl(tmp_path):
    ...
    e_priv = push_lite_row(pool, "alice", task_type="task_priv", text="priv", acl="private")
    e_team = push_lite_row(pool, "alice", task_type="task_team", text="team", acl="team:platform")
    e_org = push_lite_row(pool, "alice", task_type="task_org", text="org", acl="org")

    # Bob (different team) does not see private/team/direct-org rows.
    bob_hits = search_with_acl(pool, "bob", "task")
    bob_ids = {h["experience_id"] for h in bob_hits}
    assert e_org["experience_id"] not in bob_ids
    assert e_team["experience_id"] not in bob_ids
    assert e_priv["experience_id"] not in bob_ids

    # Carol (same team as alice) sees team but not private/direct-org.
    carol_hits = search_with_acl(pool, "carol", "task")
    carol_ids = {h["experience_id"] for h in carol_hits}
    assert e_org["experience_id"] not in carol_ids
