...
def _row_acl(pool: ExperiencePool, experience_id: str) -> tuple[str, str, str]:
    cur = pool.conn.execute(
        "SELECT agent_id, acl, COALESCE(publish_status, 'private') AS publish_status "
    ...
    return (row["agent_id"], row["acl"], row["publish_status"])


def _can_read_experience(
    viewer_agent_id: str,
    viewer_team: str,
    owner_agent_id: str,
    acl: str,
    ...
    # Historical direct-upload rows may have acl=public/org without having gone
    # through strict-public publish. Keep them owner-only until explicitly
    # published.
    if acl in {"public", "org"} and publish_status != "published":
        return viewer_agent_id == owner_agent_id
    return can_read(viewer_agent_id, viewer_team, owner_agent_id, acl)
...
def search_with_acl(
    ...
    *,
    ...
    visible = []
    for hit in raw:
        owner_id, acl, publish_status = _row_acl(pool, hit["experience_id"])
        if _can_read_experience(viewer_id, viewer_team, owner_id, acl, publish_status):
            visible.append(hit)
...
def get_with_acl(
    ...
    viewer_id, viewer_team = _viewer_context(pool, viewer_name)
    owner_id, acl, publish_status = _row_acl(pool, experience_id)
    if not owner_id:
        return None
    if not _can_read_experience(viewer_id, viewer_team, owner_id, acl, publish_status):
