...
默认域名是 `expool.internal`，上线前改成创智内网 DNS 名称。

发布后统一入口应该只暴露内网页面域名：

- `/`、`/experiences`、`/skills` 等 UI 路由转到 `127.0.0.1:3000`
- `/v1/*`、`/healthz`、`/docs`、`/openapi.json` 转到 `127.0.0.1:8080`
- `/__gateway/health` 返回网关自身健康状态

## 本地内网网关预览

本地不要再只看裸 `3000` 端口，直接用统一网关预览发布形态：
...
- 网关健康：`http://127.0.0.1:3080/__gateway/health`
- API 健康：`http://127.0.0.1:3080/healthz`
- UI 直连调试：`http://127.0.0.1:3000`
- API 直连调试：`http://127.0.0.1:8080`

如果机器已经安装 Caddy，可以切到 Caddy 本地网关：
...
如果当前 shell 配了 `http_proxy` / `https_proxy`，脚本会自动设置
...
`credentials/` 含 HMAC secret，备份目录必须保持 `0700/0600` 权限。
...
- Agent 独立 HMAC 凭据
- 本地脱敏和 rule-based 结构化
- `/v1/lite/push`
- `/v1/lite/search`
- SQLite + 向量存储
- private / team / public ACL
- 创智内网 UI

暂不启用评分、信用回流、技能市场作为主流程；相关代码仍保留。
