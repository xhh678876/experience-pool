"""DEPRECATED PROTOTYPE — not the deployed server.

This is an early, intentionally-unauthenticated sketch of the gateway kept for
reference only. The supported, authenticated server is
``core/exp_core/server.py``. Do NOT expose this module on an untrusted network:
it performs no auth and trusts the X-Agent-* headers verbatim.
"""
